...
class PaypalNvTest < Test::Unit::TestCase
  def setup
    ...
    @gateway = PaypalNvGateway.new(fixtures(:paypal_signature))  # :paypal_certificate
  ...
  # sometimes fails with "The transaction could not be loaded"
  ...
  def test_successful_reference_transaction
    purchase = @gateway.purchase(@amount, @credit_card, @options)
    assert_success purchase
    assert reference_id = purchase.authorization
    assert string_amount = purchase.params['amt']
    integer_amount = (string_amount.to_f) * 100
    response = @gateway.purchase_from_reference_transaction(reference_id, integer_amount)
    assert_success response
  end
  ...
  # failing with "You can not do a partial refund on this transaction"
  ...
  # sometimes fails with "Timeout processing request"
